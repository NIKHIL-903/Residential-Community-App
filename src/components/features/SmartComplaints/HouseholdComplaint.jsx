...
export default function HouseholdComplaint() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label className="block text-sm font-medium text-ink mb-2">Complaint</label>
        <textarea
          value={complaint}
          ...
          placeholder="Describe your household complaint..."
        />
      </div>
      <div>
        <label className="block text-sm font-medium text-ink mb-2">Image upload (UI only)</label>
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
